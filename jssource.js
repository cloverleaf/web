...
function process() {
	...
	while (true) {
		...
		if (requirements !== []) {
			let nope = false;
			for (let j = 0; j < requirements.length; j++) { // For each requirement
				for (let c = 0; c < requirements[j].length; c++) { // For each character in the requirement group
...
window.onload = function () {
	...
	themes.onload = function () {
		...
		for (const key in themeData) {
			...
			const css = `a[id=${key}]{background-color: ${themeData[key].background} !important; border: 1px ;}a[id=${key}] i {color: ${themeData[key].text}`;
...
function colourUnderline() {
	...
	if (document.getElementById("pass").value) {
		...
		const colour = `HSL(${getRandomArbitrary(0, 360)}, ${getRandomArbitrary(60, 100)}%, ${getRandomArbitrary(45, 80)}%)`;
		document.getElementById("pass").style.setProperty("--underlineColor", colour);
	} else {
